...
tranlate_api = 'https://fanyi.baidu.com/v2transapi?from=zh&to=en'
...
def get_sign(query):
    with open('test.js', 'r', encoding='utf-8') as f:
        test_js = f.read()
    sign = execjs.compile(test_js).call('e', query)
    return sign
...
def get_result(lang, query, sign, token):
    ...
    response = requests.post(url=tranlate_api, headers=headers, data=data)
    # print(response.json())
